...
class TestProjectTask(common.SavepointCase):
    ...
    def test_whenPrAreMerged_thenTaskMerged(self):
        task = self.env["project.task"].create(
            {
                "name": "ttask",
                "pull_request_ids": [(6, False, (self.pull_request_merged.id,))],
            }
        )
    ...
    def test_whenPrAreClosed_thenTaskClosed(self):
        task = self.env["project.task"].create(
            {
                "name": "ttask",
                "pull_request_ids": [(6, False, (self.pull_request_closed.id,))],
            }
        )
    ...
    def test_whenPrAreOpen_thenTaskOpen(self):
        task = self.env["project.task"].create(
            {
                "name": "ttask",
                "pull_request_ids": [(6, False, (self.pull_request_open.id,))],
            }
        )
        ...
        task = self.env["project.task"].create(
            {
                ...
                "pull_request_ids": [
                    (
                        6,
                        False,
                        (self.pull_request_closed.id, self.pull_request_merged.id),
                    )
                ],
            }
        )
